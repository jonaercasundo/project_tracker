...
import { sanitizeBlockedOutputGuardrailResults, sanitizeBlockedToolOutputGuardrailResults, } from "./guardrails.mjs";
...
export function sanitizeBlockedTerminalToolOutput(state, outputGuardrailResultStart, completedOutputGuardrailTripwireResult, observedOutputGuardrailResults, tripwire, ownedOutputGuardrailResults) {
    if (!hasTerminalToolOutputSource(state) ||
        !completedOutputGuardrailTripwireResult) {
        return false;
    }
    redactBlockedResponseToolOutputs(state);
    sanitizeBlockedOutputGuardrailResults(state, outputGuardrailResultStart, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT, observedOutputGuardrailResults, tripwire, ownedOutputGuardrailResults);
...
export function assertResumedSessionOutputGuardrailSafety(state, session, hasOutputGuardrail) {
    ...
    if (state._serializedCurrentStep !== undefined && shouldDefer) {
        ...
        if (session !== undefined ||
            ...
            throw new UserError('Cannot resume this serialized output-bearing approval checkpoint because current-response provenance was not preserved. Start a new run from safe input.');
        }
    }
    if (session === undefined) {
    ...
    const currentStart = state._generatedItems.findIndex((item) => item instanceof RunToolCallItem &&
...
export function buildBlockedToolOutputRawItem(rawItem) {
...
function currentRunItemSelection(state, responseOutput) {
    ...
    if (state._currentStep?.type === 'next_step_interruption' &&
        ...
        processed.every((item, index) => item instanceof RunToolCallItem &&
            item.rawItem.type === 'function_call' &&
    ...
    try {
        ...
        for (const [index, item] of selection.primary.entries()) {
            if (item instanceof RunToolCallItem &&
                ...
                sanitizedItems.push(new RunToolCallItem(buildCanonicalFunctionCall(item.rawItem), item.agent));
                continue;
            }
            if (item instanceof RunToolCallOutputItem &&
                item.rawItem.type === 'function_call_result') {
                ...
                sanitizedItems.push(new RunToolCallOutputItem(buildBlockedToolOutputRawItem(item.rawItem), item.agent, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT, undefined, item.executionStatus));
                continue;
            }
            return undefined;
        }
        ...
        if (currentProcessedStart >= 0) {
            ...
            for (const [index, item] of currentProcessed.entries()) {
                ...
                if (replacement instanceof RunToolCallItem &&
                    ...
                    replacements.set(item, new RunToolCallItem(replacement.rawItem, item.agent));
                }
                else if (replacement instanceof RunToolCallOutputItem &&
                    item instanceof RunToolCallOutputItem) {
                    replacements.set(item, new RunToolCallOutputItem(replacement.rawItem, item.agent, OUTPUT_GUARDRAIL_BLOCKED_TOOL_OUTPUT, undefined, replacement.executionStatus));
                }
                else {
                    return undefined;
                }
...
export function redactBlockedResponseToolOutputs(state) {
