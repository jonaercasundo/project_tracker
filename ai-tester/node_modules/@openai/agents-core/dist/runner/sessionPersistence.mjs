...
import { isOpenAIResponsesCompactionAwareSession, isRunContextAwareSession, isSessionHistoryTransactionAwareSession, } from "../memory/session.mjs";
...
import { buildRunItemPersistencePlan as buildCanonicalRunItemPersistencePlan, getBlockedOutputSessionSnapshotRunItems, hasBlockedOutputExecutionEffect, selectRunItemIndexesForBlockedOutput, } from "./blockedOutputPersistence.mjs";
...
export async function saveStreamInputToSession(session, sessionInputItems, runContext) {
    if (!session) {
        return;
    }
...
export async function prepareInputItemsWithSession(input, session, sessionInputCallback, options, runContext) {
    ...
    const history = trimToLatestCompaction(await getSessionItems(session, runContext));
