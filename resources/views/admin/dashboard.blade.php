    ...
    <div class="space-y-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">
                        Here's what's happening across your team today &bull; <span class="font-semibold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md text-xs uppercase tracking-wide border border-rose-100">Admin</span>
                    </p>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between group hover:border-blue-300 transition-colors">
                <div>
                    <span class="text-xs font-bold uppercase tracking-wider text-slate-400 block">Total People</span>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between group hover:border-indigo-300 transition-colors">
                <div>
                    <span class="text-xs font-bold uppercase tracking-wider text-slate-400 block">Managers &amp; Admins</span>
                    <span class="text-3xl font-extrabold text-slate-900 block mt-2">
        ...
        <div class="bg-white border border-slate-200/80 shadow-sm rounded-2xl p-6">
            <h3 class="text-lg font-bold text-slate-900 tracking-tight mb-1">Quick actions</h3>
            ...
            <div class="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden lg:col-span-2 flex flex-col">
                ...
                <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">See who's on the team and manage what they can access.</p>
                    </div>
                    
                    <div class="flex items-center flex-wrap sm:flex-nowrap gap-3">
                        {{-- Search bar: submits via GET so results, pagination, and query string all stay in sync --}}
                        <form method="GET" action="{{ url()->current() }}" class="relative w-full sm:w-64">
                            ...
                            @if(request('search'))
                                <a href="{{ url()->current() }}" class="absolute right-2.5 top-2.5 text-slate-400 hover:text-slate-600" title="Clear search">
                        ...
                        <div x-data="{ open: false }">
                            <button @click="open = true" class="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition-colors shadow-sm shadow-blue-500/10 whitespace-nowrap">
                            ...
                            >
                                ...
                                >
                                    <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-4 shrink-0">
                                        <div>
                                            <h2 class="text-base font-bold text-slate-900">Add a new person</h2>
                                            <p class="text-[11px] text-slate-400 font-normal mt-0.5">Fill in their details to create an account.</p>
                                        </div>
                ...
                <div class="w-full overflow-x-auto rounded-2xl border border-slate-200/80 bg-white" x-data="{ activeModal: null }">
            ...
            @forelse($users as $user)
                ...
                <tr class="hover:bg-slate-50/60 transition-colors duration-150">
                    ...
                    <td class="py-3.5 px-4">
                        <span class="block text-slate-900 font-medium">{{ $user->department ?? '—' }}</span>
                        <span class="block text-[10px] text-slate-400 font-normal mt-0.5">{{ $user->position ?? 'No title set' }}</span>
                    </td>
                    ...
                    <td class="py-3.5 px-4">
                        <div class="flex flex-wrap gap-1">
                            ...
                            @empty
                                <span class="text-[11px] text-slate-400 font-normal italic">No role yet</span>
                            @endforelse
                        </div>
                    </td>
                    ...
                    <td class="py-3.5 px-6 text-right whitespace-nowrap">
                        <div class="flex justify-end items-center gap-3">
                            <button @click="activeModal = 'edit-{{ $rowKey }}'" 
                            ...
                            <button @click="activeModal = 'delete-{{ $rowKey }}'" 
                        ...
                        <div x-show="activeModal === 'edit-{{ $rowKey }}'" x-cloak 
                            ...
                            <div @click.away="activeModal = null" 
                                ...
                                <h3 class="text-base font-bold text-slate-900 mb-4">Update this person's role</h3>
                                ...
                                <form method="POST" action="{{ route('roleaccess.update') }}" class="space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-slate-500 mb-1">Editing</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-slate-700 mb-2">Roles</label>
                                        ...
                                        <p class="text-[10px] text-slate-400 mt-1.5">You can pick more than one. This replaces their current roles with whatever's checked here.</p>
                                    </div>
                        ...
                        <div x-show="activeModal === 'delete-{{ $rowKey }}'" x-cloak 
                            class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 text-left font-normal"
                            x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0"
                            x-transition:enter-end="opacity-100"
                            ...
                            <div @click.away="activeModal = null" 
                                ...
                                <h3 class="text-base font-bold text-slate-900 mb-2">Remove this person?</h3>
                                <p class="text-xs text-slate-500 leading-relaxed mb-5">
                                    You're about to remove <span class="font-semibold text-slate-800">{{ $user->name }}</span> from the system. Their activity history will still be kept on record.
                                </p>
            ...
            @empty
                <tr>
                    <td colspan="6" class="py-12 text-center text-slate-400 font-normal select-none">
                        ...
                        @if(request('search'))
                            <span class="text-xs font-medium text-slate-400">No one matches "{{ request('search') }}". Try a different search.</span>
                        @else
                            <span class="text-xs font-medium text-slate-400">No one's been added yet.</span>
                        @endif
                    </td>
                </tr>
            @endforelse
                ...
                {{-- Real pagination, driven by $users->paginate() in the controller --}}
                <div class="p-4 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
                    <span class="font-medium">
                        @if($users->total() > 0)
                            Showing {{ $users->firstItem() }}–{{ $users->lastItem() }} of {{ $users->total() }} {{ Str::plural('person', $users->total()) }}
                        @else
                            No results
                        @endif
                    </span>
                    <div class="flex items-center gap-2">
                        @if ($users->onFirstPage())
                            <span class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-300 cursor-not-allowed font-semibold">Previous</span>
                        @else
                            <a href="{{ $users->previousPageUrl() }}" class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-100 font-semibold transition-colors">Previous</a>
                        @endif

                        @if ($users->hasMorePages())
                            <a href="{{ $users->nextPageUrl() }}" class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-100 font-semibold transition-colors">Next</a>
                        @else
                            <span class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-300 cursor-not-allowed font-semibold">Next</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 flex flex-col h-fit">
                ...
                <div class="space-y-4 flex-grow overflow-y-auto max-h-[340px] pr-1">
                    <div class="flex gap-3 text-xs border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                        ...
                        <div>
                            <p class="font-medium text-slate-600"><span class="font-semibold text-slate-900">John Doe</span> logged in</p>
                            <span class="text-[10px] font-mono text-slate-400 block mt-0.5">IP: 192.168.1.42 &bull; 4 minutes ago</span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3 text-xs border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                        ...
                        <div>
                            <p class="font-medium text-slate-600"><span class="font-semibold text-slate-900">New account created</span> through the sign-up page</p>
                    ...
                    <div class="flex gap-3 text-xs border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                        ...
                        <div>
                            <p class="font-medium text-slate-600"><span class="font-semibold text-slate-900">Settings updated</span> by an admin</p>
                            <span class="text-[10px] font-mono text-slate-400 block mt-0.5">By: System Admin &bull; 3 hours ago</span>
                        </div>
                    </div>
                </div>
