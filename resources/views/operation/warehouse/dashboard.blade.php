...
const WAREHOUSE_ID = 1; // TODO: replace with actual selected warehouse
...
scannerInput.addEventListener('keydown', async function (e) {
    ...
    try {
        ...
        if (ok) {
            scannedList.add(qr); // only mark as "seen" once it actually succeeded
        }
    } finally {
        isProcessing = false;
    }
...
async function validateScan(qr) {
    try {
        ...
        if (response.ok && result.success) {
            ...
            failedCounter++;
            const msg = result.message ?? (result.errors ? Object.values(result.errors).flat().join(', ') : 'Failed');
            addRow({
                package: result.package ?? '-',
                item: result.item ?? '-',
                qty: result.qty ?? '-',
